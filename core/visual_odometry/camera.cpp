...
void Camera::initParams(int n_cols, int n_rows, const cv::Mat &cvK, const cv::Mat &cvD)
{
	...
	fx_ = cvK.at<float>(0, 0);
	fy_ = cvK.at<float>(1, 1);
	cx_ = cvK.at<float>(0, 2);
	cy_ = cvK.at<float>(1, 2);
	fxinv_ = 1.0f / fx_;
	fyinv_ = 1.0f / fy_;
...
void Camera::generateImageUndistortMaps()
{
	...
	for (int v = 0; v < n_rows_; ++v)
	{
		...
		y = (static_cast<float>(v) - cy_) * fyinv_;

		for (int u = 0; u < n_cols_; ++u)
		{
			x = (static_cast<float>(u) - cx_) * fxinv_;
			xy2 = 2.0 * x * y;
			xx = x * x;
			yy = y * y;
			r2 = xx + yy;
...
{
	constexpr float THRES_EPS = 1e-9;
	...
	for (int v = 0; v < n_rows_; ++v)
	{
		...
		yd = (static_cast<float>(v) - cy_) * fyinv_;

		float err_prev = 1e12;
		for (int u = 0; u < n_cols_; ++u)
		{
			xd = (static_cast<float>(u) - cx_) * fxinv_;
			...
			for (int iter = 0; iter < MAX_ITER; ++iter)
			{
				...
				if (fabs(err_curr - err_prev) < THRES_EPS)
					break;
...
{
	if (!is_initialized_to_stereo_rectify_)
		throw std::runtime_error("In 'getRectifiedCamera()', is_initialized_to_stereo_rectify_ == false");
	return cam_rect_;
}
...
{
	if (!is_initialized_to_stereo_rectify_)
		throw std::runtime_error("In 'rectifyStereoImages()', is_initialized_to_stereo_rectify_ == false");
	...
	if (img_left.channels() == 3)
		...
		img_left.copyTo(img_left_float);
	...
	if (img_right.channels() == 3)
		...
		img_right.copyTo(img_right_float);
...
{
	if (!is_initialized_to_stereo_rectify_)
		throw std::runtime_error("In 'getRectifiedStereoPoseLeft2Right()', is_initialized_to_stereo_rectify_ == false");
...
{
	if (!is_initialized_to_stereo_rectify_)
		throw std::runtime_error("In 'getRectifiedStereoPoseRight2Left()', is_initialized_to_stereo_rectify_ == false");
...
void StereoCamera::generateStereoImagesUndistortAndRectifyMaps()
{
	...
	const auto &n_cols = cam_left_->cols();
	const auto &n_rows = cam_left_->rows();
	...
	const float centv = static_cast<float>(n_rows) * 0.5f;
	...
	const auto &fx_l = cam_left_->fx();
	const auto &fy_l = cam_left_->fy();
	const auto &cx_l = cam_left_->cx();
	const auto &cy_l = cam_left_->cy();

	const auto &fx_r = cam_right_->fx();
	const auto &fy_r = cam_right_->fy();
	const auto &cx_r = cam_right_->cx();
	const auto &cy_r = cam_right_->cy();
