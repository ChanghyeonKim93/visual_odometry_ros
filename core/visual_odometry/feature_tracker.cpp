...
void FeatureTracker::track(const cv::Mat &img0, const cv::Mat &img1, const PixelVec &pts0, int window_size, int max_pyr_lvl, float thres_err,
...
void FeatureTracker::trackBidirection(const cv::Mat &img0, const cv::Mat &img1, const PixelVec &pts0, int window_size, int max_pyr_lvl, float thres_err, float thres_bidirection,
                                      PixelVec &pts_track, MaskVec &mask_valid)
{
    ...
    for (int i = 0; i < n_pts; ++i)
    {
        ...
        // border validity
        mask_valid[i] = (mask_valid[i] && pts_track[i].x > 3 && pts_track[i].x < n_cols - 3 && pts_track[i].y > 3 && pts_track[i].y < n_rows - 3);
        // other ...
        mask_valid[i] = (mask_valid[i] && status_forward[i] && status_backward[i] && err_forward[i] <= thres_err && err_backward[i] <= thres_err && dist2 <= thres_bidirection2);
    }

    // printf(" - FEATURE_TRACKER - 'trackBidirection()'\n");
};

void FeatureTracker::trackBidirectionWithPrior(const cv::Mat &img0, const cv::Mat &img1, const PixelVec &pts0, int window_size, int max_pyr_lvl, float thres_err, float thres_bidirection,
                                               PixelVec &pts_track, MaskVec &mask_valid)
{
    ...
    for (int i = 0; i < n_pts; ++i)
    {
        ...
        bool flag_inimage = pts_track[i].x > 0 && pts_track[i].x < n_cols && pts_track[i].y > 0 && pts_track[i].y < n_rows;
        ...
        mask_valid[i] = (mask_valid[i] && flag_inimage && flag_forward && flag_forward_err && flag_backward && flag_backward_err && flag_bidir);
...
void FeatureTracker::trackWithPrior(const cv::Mat &img0, const cv::Mat &img1, const PixelVec &pts0, int window_size, int max_pyr_lvl, float thres_err,
                                    PixelVec &pts_track, MaskVec &mask_valid)
{
    int n_cols = img0.size().width;
    int n_rows = img0.size().height;
    ...
    {
        // Border check
        mask_valid[i] = (mask_valid[i] && status[i] > 0 && pts_track[i].x > 0 && pts_track[i].x < n_cols && pts_track[i].y > 0 && pts_track[i].y < n_rows);
        // Error check
                               ...
                               PixelVec &pts1_prior)
{
    ...
    PointVec X1;
    X1.reserve(n_pts);
    for (const auto &Xw_ : Xw)
        X1.emplace_back((T1w.block(0, 0, 3, 3) * Xw_ + T1w.block(0, 3, 3, 1)));
        ...
        for (int iter = 0; iter < MAX_ITER; ++iter)
        {
            ...
            for (int j = 0; j < n_elem; ++j)
            {
                if (mask_I0[j] && mask_I1[j])
                {
                    ...
                    if (std::isnan(I0_tmp) || std::isnan(I1_tmp))
                    {
                    ...
                    if (std::isnan(du0_tmp) || std::isnan(dv0_tmp))
                    {
            ...
            if (iter > 1)
            {
                if (err_rate <= EPS_ERR_RATE || dt_norm <= EPS_UPDATE)
                    break;
            }
