...
bool Keyframes::checkUpdateRule(const FramePtr &frame_curr)
{
	...
	if (!flag_addkeyframe)
	{
		...
		for (std::list<FramePtr>::iterator it = kfs_list_.begin(); it != kfs_list_.end(); ++it)
		{
			...
			const auto tracking_ratio = static_cast<float>(cnt_tracked) / static_cast<float>(cnt_total_lms);
		...
		{
			std::cout << static_cast<int>(r * 100.f) << "% ";
		}
		std::cout << "\n";
		...
		if (!flag_addkeyframe)
		{
			...
			if (costheta <= -0.999999f)
				costheta = -0.999999f;
			const float rot = acosf(costheta);

			const float dtrans = dT.block<3, 1>(0, 3).norm();
...
void StereoKeyframes::addNewStereoKeyframe(const StereoFramePtr &stframe)
{
	...
	for (size_t i = 0; i < lms_left.size(); ++i)
	{
	...
	for (size_t i = 0; i < lms_right.size(); ++i)
	{
	...
	if (!flag_addkeyframe)
	{
		...
		for (std::list<StereoFramePtr>::iterator it = stereo_kfs_list_.begin(); it != stereo_kfs_list_.end(); ++it)
		{
			...
			for (const auto &lm : frame_left->getRelatedLandmarkPtr())
			{
				...
				const int len = static_cast<int>(lm->getRelatedFramePtr().size());
				if (len - 1 < 0 || len - 2 < 0)
					throw std::runtime_error("In checkUpdateRule(), if(len-1 <0 || len-2 < 0)");
				if (lm->getRelatedFramePtr()[len - 1] == stframe_curr->getLeft() || lm->getRelatedFramePtr()[len - 1] == stframe_curr->getRight() || lm->getRelatedFramePtr()[len - 2] == stframe_curr->getLeft() || lm->getRelatedFramePtr()[len - 2] == stframe_curr->getRight())
					++cnt_tracked; // kf의 landmark가 현재 프레임으로 추적 되었는지.
			}
