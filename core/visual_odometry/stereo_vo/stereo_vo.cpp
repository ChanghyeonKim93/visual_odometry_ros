...
void StereoVO::loadStereoCameraIntrinsicAndUserParameters(const std::string &dir)
{
	...
	if (!fs.isOpened())
		throw std::runtime_error("stereo intrinsic file cannot be found!\n");

	// Left camera
	int rows, cols;
	rows = fs["Camera.left.height"];
	cols = fs["Camera.left.width"];

	float fx, fy, cx, cy;
	fx = fs["Camera.left.fx"];
	fy = fs["Camera.left.fy"];
	cx = fs["Camera.left.cx"];
	cy = fs["Camera.left.cy"];

	float k1, k2, k3, p1, p2;
	k1 = fs["Camera.left.k1"];
	k2 = fs["Camera.left.k2"];
	k3 = fs["Camera.left.k3"];
	p1 = fs["Camera.left.p1"];
	p2 = fs["Camera.left.p2"];
	...
	// Right camera
	rows = fs["Camera.right.height"];
	cols = fs["Camera.right.width"];

	fx = fs["Camera.right.fx"];
	fy = fs["Camera.right.fy"];
	cx = fs["Camera.right.cx"];
	cy = fs["Camera.right.cy"];

	k1 = fs["Camera.right.k1"];
	k2 = fs["Camera.right.k2"];
	k3 = fs["Camera.right.k3"];
	p1 = fs["Camera.right.p1"];
	p2 = fs["Camera.right.p2"];
		...
		const cv::Mat &img_left, const cv::Mat &img_right, const double &timestamp)
{
	...
	StereoFramePtr stframe_curr = std::make_shared<StereoFrame>(cam_left, cam_right, timestamp);
	...
	if (this->system_flags_.flagFirstImageGot)
	{
		// 초기화가 된 상태에서 반복적 구동.
		// 이전 이미지 가져오기
		const cv::Mat &I0_left = previous_left_image_;
		const cv::Mat &I0_right = previous_right_image_;

		// 현재 이미지 가져오기
		const cv::Mat &I1_left = current_left_image;
		const cv::Mat &I1_right = current_right_image;
		...
		for (int i = 0; i < n_pts_exist; ++i)
		{
			...
			if (lm->isTriangulated())
			{
				...
				if (!cam_left->inImage(pt_l1) || !cam_right->inImage(pt_r1) || X_l1(2) < 0.1 || X_r1(2) < 0.1)
				{
					// out of image.
		...
		*/
		...
		constexpr int kKernerSize = 3;
		cv::Mat du0_left, dv0_left;
		cv::Sobel(previous_left_image_, du0_left, CV_32FC1, 1, 0, kKernerSize, 1.0, 0.0, cv::BORDER_DEFAULT);
		cv::Sobel(previous_left_image_, dv0_left, CV_32FC1, 0, 1, kKernerSize, 1.0, 0.0, cv::BORDER_DEFAULT);
