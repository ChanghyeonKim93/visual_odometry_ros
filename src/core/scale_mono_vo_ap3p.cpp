 ...
 * @date 10-July-2022
 */
void ScaleMonoVO::trackImageAP3P(const cv::Mat& img, const double& timestamp){
	// Generate statistics
...
statcurr_landmark.n_ok_parallax = cnt_parallax_ok;
			...
			if(cnt_depth_ok >= 100){
				...
				MaskVec maskvec_p3p;
				if(motion_estimator_->calcPosePnPAlgorithm(X_depth_ok,pts1_depth_ok,cam_,Rwc_est,twc_est,maskvec_p3p)){
					PoseSE3 Twc_ba;
					Twc_ba <<Rwc_est, twc_est,0,0,0,1;
					frame_curr->setPose(Twc_ba);
				}
				else{
					std::cout << "  AP3P failed...\n";
				}
...
statcurr_frame.dT_01 = frame_curr->getPoseDiff01();
			...
			for(auto lm : lms1_final){
				if(!lm->isTriangulated()){
					...
					if(lm->getMaxParallax() >= 10.0f*D2R){
